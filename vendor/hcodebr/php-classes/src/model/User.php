...
class User extends Model
{
	...
	const SECRET = "HcodePhp7_Secret";//criando chave que será usada no mbcrypt_encode
	...
	public function get($iduser)
	{
 	...
 	public static function getForgot($email)
 	{
 		...
 		if (count($results) === 0) {
 			...
 			$data = $results[0]; // pegando o id do usuario que está na posição 0 do array

 			$results2 = $sql->select("CALL sp_userspasswordsrecoveries_create(:iduser, :desip)", array(
 				":iduser"=>$data['iduser'],
 				":desip"=>$_SERVER['REMOTE_ADDR']//Isso retorna o ip do úsuario
 			));
			...
			if (count($results2) === 0 ) {
				...
				//criptografando mensangem que sera enviada como link para recuperação com mcrypt_encrypt(cipher, key, data, mode)
				//openssl_encrypt(data, method, password)
				$code = openssl_encrypt($dataRecovery['idrecovery'],'AES-128-CBC', User::SECRET, 0,  pack("a16", User::SECRET_IV));
